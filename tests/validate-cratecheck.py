...
"""
                ...
                try:
                    ...
                    # --- Prove the behavioral kill-check FAILS on mutated output ---
                    # Re-run the same logic as the parent behavioral check:
                    #   "trap actually calls kill on PF_PID (behavioral)" expects
                    #   "kill 424242" in trace.  On the mutated trace it should not
                    #   be there, so the behavioral check MUST return False.
                    all_ok &= check(
                        "runbook fixtures: adversarial M5 — behavioral kill-check FAILS on mutated trace (validator would exit nonzero)",
                        not ("kill 424242" in mutated_trace_content),
                        "kill correctly absent — behavioral kill-check would fail, validator would exit nonzero"
                        if not ("kill 424242" in mutated_trace_content)
                        else "kill STILL present — behavioral kill-check would accidentally pass",
                    )
                finally:
...
"""
                ...
                try:
                    ...
                    # Mutation should have removed the wait call
                    all_ok &= check(
                    ...
                    # --- Prove the behavioral wait-check FAILS on mutated output ---
                    all_ok &= check(
                        "runbook fixtures: adversarial M8 — behavioral wait-check FAILS on mutated trace (validator would exit nonzero)",
                        not ("wait 424242" in wait_trace_content),
                        "wait correctly absent — behavioral wait-check would fail, validator would exit nonzero"
                        if not ("wait 424242" in wait_trace_content)
                        else "wait STILL present — behavioral wait-check would accidentally pass",
                    )
                finally:
...
"""
                ...
                try:
                    ...
                    # --- Prove the behavioral reconcile-failure check FAILS ---
                    # The parent check asserts exit != 0 for injected reconcile
                    # failure.  With reconcile removed, the trap exits 0, so the
                    # behavioral check MUST return False.
                    all_ok &= check(
                        "runbook fixtures: adversarial M6 — behavioral reconcile-failure check FAILS on mutated trap (validator would exit nonzero)",
                        result_m6.returncode == 0,
                        "trap exits 0 — behavioral reconcile-failure check would fail, validator would exit nonzero"
                        if result_m6.returncode == 0
                        else f"exit={result_m6.returncode} — behavioral check would accidentally pass",
                    )
                finally:
...
"""
                ...
                try:
                    ...
                    # --- Prove the behavioral read-failure check FAILS ---
                    # The parent check asserts exit != 0 for injected
                    # post-reconcile resource-read failure.  With the read
                    # removed, the trap exits 0, so the behavioral check
                    # MUST return False.
                    all_ok &= check(
                        "runbook fixtures: adversarial M7 — behavioral read-failure check FAILS on mutated trap (validator would exit nonzero)",
                        result_m7.returncode == 0,
                        "trap exits 0 — behavioral read-failure check would fail, validator would exit nonzero"
                        if result_m7.returncode == 0
                        else f"exit={result_m7.returncode} — behavioral check would accidentally pass",
                    )
                finally:
...
"""
    ...
    if mutated_recipe_neg != recipe:
        ...
        # --- Prove the structural override-path check FAILS on mutated recipe ---
        # The parent validator asserts override path creates ConfigMap and
        # does NOT delete.  With condition inverted, neither holds.
        all_ok &= check(
            "flux sync override reset: adversarial M1 — structural override-create check FAILS on mutated recipe (validator would exit nonzero)",
            not m1_has_create,
            "create absent — structural check 'override path creates ConfigMap' would fail"
            if not m1_has_create
            else "create STILL present — structural check would accidentally pass",
        )
        ...
        # --- Prove the structural default-delete check FAILS ---
        all_ok &= check(
            "flux sync override reset: adversarial M1 — structural default-delete check FAILS on mutated recipe (validator would exit nonzero)",
            not m1d_has_delete,
            "delete absent — structural check 'default path deletes ConfigMap' would fail"
            if not m1d_has_delete
            else "delete STILL present — structural check would accidentally pass",
        )
    else:
        all_ok &= check(
    ...
    if mutated_no_create != recipe:
        ...
        # --- Prove the structural create check FAILS on mutated recipe ---
        m2_recipe_has_create = "create configmap flux-sync-values-override" in mutated_no_create
        all_ok &= check(
            "flux sync override reset: adversarial M2 — structural 'recipe has create command' check FAILS on mutated recipe (validator would exit nonzero)",
            not m2_recipe_has_create,
            "create absent from mutated recipe — structural check would fail"
            if not m2_recipe_has_create
            else "create STILL in mutated recipe — structural check would accidentally pass",
        )
    else:
        all_ok &= check(
    ...
    if mutated_no_apply != recipe:
        ...
        # --- Prove the structural apply check FAILS on mutated recipe ---
        m3_recipe_has_apply = "apply -f -" in mutated_no_apply
        all_ok &= check(
            "flux sync override reset: adversarial M3 — structural 'recipe has apply -f -' check FAILS on mutated recipe (validator would exit nonzero)",
            not m3_recipe_has_apply,
            "apply absent from mutated recipe — structural check would fail"
            if not m3_recipe_has_apply
            else "apply STILL in mutated recipe — structural check would accidentally pass",
        )
    else:
        all_ok &= check(
    ...
    if mutated_no_delete != recipe:
        ...
        # --- Prove the structural delete check FAILS on mutated recipe ---
        m4_recipe_has_delete = "delete configmap flux-sync-values-override" in mutated_no_delete
        all_ok &= check(
            "flux sync override reset: adversarial M4 — structural 'recipe has delete command' check FAILS on mutated recipe (validator would exit nonzero)",
            not m4_recipe_has_delete,
            "delete absent from mutated recipe — structural check would fail"
            if not m4_recipe_has_delete
            else "delete STILL in mutated recipe — structural check would accidentally pass",
        )
    else:
        all_ok &= check(
    ...
    # ===================================================================
    # Prove one lifecycle with persisted effective branch through simulated
    # reconciliation.  Phase 1: candidate override → assert candidate active →
    # simulate reconciliation → candidate still active.  Phase 2: default
    # bootstrap → assert override removed → simulate reconciliation → exact
    # pivot/flux-sync-ssh-bootstrap restored.

    stateful_trace = tmp_dir / "stateful-lifecycle-trace.txt"
    stateful_trace.write_text("")

    # Shared mock scripts that persist state across both phases via the trace file
...
# Record effective branch for reconciliation simulation
case "$*" in
    *"create configmap flux-sync-values-override"*)
        echo "STATE:override-created" >> "{stateful_trace}"
        ;;
    *"delete configmap flux-sync-values-override"*)
        echo "STATE:override-deleted" >> "{stateful_trace}"
        ;;
esac
exit 0
""")
...
""")
    ...
    def _run_stateful_phase(override_branch: str, phase_label: str) -> tuple[int, str]:
...
"""
        ...
        return result.returncode, trace_snapshot

    # --- Phase 1: Candidate bootstrap ---
    exit_cand, trace_cand = _run_stateful_phase("candidate/test-stateful", "candidate")
    cand_created = "STATE:override-created" in trace_cand
    all_ok &= check(
    ...
    all_ok &= check(
        "flux sync override reset: stateful lifecycle phase-1 — override ConfigMap created",
        cand_created,
        "STATE:override-created found in trace" if cand_created
        else f"trace: {trace_cand.strip()[-300:]}",
    )

    # --- Simulate Flux reconciliation after phase 1 ---
    # Reconciliation re-applies the committed config; since the override
    # ConfigMap exists, it persists (it is not managed by kustomize).
    # The effective branch should remain candidate.
    all_ok &= check(
        "flux sync override reset: stateful lifecycle after phase-1 reconcile — override persists (candidate remains effective)",
        cand_created,
        "STATE:override-created still present — candidate survives reconciliation"
        if cand_created
        else "override missing — candidate would be lost after reconcile",
    )

    # --- Phase 2: Default bootstrap (overrides empty) ---
    exit_def, trace_def = _run_stateful_phase("", "default")
    def_deleted = "STATE:override-deleted" in trace_def
    all_ok &= check(
    ...
    all_ok &= check(
        "flux sync override reset: stateful lifecycle phase-2 — override ConfigMap deleted",
        def_deleted,
        "STATE:override-deleted found in trace" if def_deleted
        else f"trace: {trace_def.strip()[-300:]}",
    )

    # --- Simulate Flux reconciliation after phase 2 ---
    ...
    all_ok &= check(
        "flux sync override reset: stateful lifecycle after phase-2 reconcile — canonical branch is pivot/flux-sync-ssh-bootstrap",
        def_deleted,
        "override deleted — reconciliation resolves to pivot/flux-sync-ssh-bootstrap"
        if def_deleted
        else "override still present — canonical branch not restored",
    )
