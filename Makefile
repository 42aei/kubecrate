...
> kubectl --context "$(KIND_CONTEXT)" wait --for=condition=Available deployment/kustomize-controller -n "$(FLUX_NAMESPACE)" --timeout=180s
> kubectl --context "$(KIND_CONTEXT)" wait --for=condition=Available deployment/helm-controller -n "$(FLUX_NAMESPACE)" --timeout=180s
> expected_url="$$(python3 -c 'import re,sys; text=open(sys.argv[1]).read(); print(re.search(r"(?m)^    url: (\S+)", text).group(1))' "$(FLUX_SYNC_VALUES)")"; if [ -n "$(FLUX_GIT_BRANCH_OVERRIDE)" ]; then expected_branch="$(FLUX_GIT_BRANCH_OVERRIDE)"; printf 'Using FLUX_GIT_BRANCH_OVERRIDE: %s\n' "$${expected_branch}"; else expected_branch="$$(python3 -c 'import re,sys; text=open(sys.argv[1]).read(); print(re.search(r"(?m)^      branch: (\S+)", text).group(1))' "$(FLUX_SYNC_VALUES)")"; fi; live_url="$$(kubectl --context "$(KIND_CONTEXT)" get gitrepository "$(FLUX_RELEASE_NAME)" -n "$(FLUX_NAMESPACE)" -o jsonpath='{.spec.url}' 2>/dev/null || true)"; live_branch="$$(kubectl --context "$(KIND_CONTEXT)" get gitrepository "$(FLUX_RELEASE_NAME)" -n "$(FLUX_NAMESPACE)" -o jsonpath='{.spec.ref.branch}' 2>/dev/null || true)"; if [ "$${live_url}" != "$${expected_url}" ] || [ "$${live_branch}" != "$${expected_branch}" ]; then printf 'GitRepository/flux-system is not reconciling the reviewed implementation source.\nexpected url: %s\nlive url: %s\nexpected branch: %s\nlive branch: %s\n' "$${expected_url}" "$${live_url}" "$${expected_branch}" "$${live_branch}" >&2; exit 1; fi
