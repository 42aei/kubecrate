...
The reusable public Vanilla composition is rooted at `compositions/vanilla/entrypoint/`. Concrete consumer repository entrypoints reference that public composition rather than keeping a separate shadow set of platform-service and application-service bindings. Consumer repositories own concrete bootstrap, Flux self-management, and cluster-specific overrides that are not part of the reusable Vanilla contract.
