...
Alternatively, run the preflight at `scripts/preflight-flux-deploy-key.sh`, which detects the downstream symptom (existing keys with `enabled: false`, or HTTP 422 on key creation) and guides the operator to the settings page.
...
4. If only a specific key is disabled, re-enable it:
   ...
   - Find the key and toggle it back on
   - Or use a `PATCH` API call

### Re-enabling after org policy change

After the org policy is restored (deploy keys allowed):

1. Re-enable existing keys via the repository settings page or API.
