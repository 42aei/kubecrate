...
### Requirement: Keep install-flow artifact docs-only
Kubecrate SHALL keep this install-flow definition docs-only and SHALL NOT add Kubernetes manifests, installation scripts, Helm charts, or final component selections.

#### Scenario: Definition work avoids implementation artifacts
- **WHEN** the install-flow change is applied
