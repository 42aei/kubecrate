...
- [x] 1.1 Define each platform service as a separately targetable GitOps-managed management unit in the spec.
- [x] 1.2 Define the management-unit contract requirements: independent installation, environment-specific configuration, avoidance of umbrella bundle lock-in.
- [x] 1.3 Document the management-unit contract in a new `docs/gitops-component-management.md` reference document.
- [x] 1.4 Add spec scenario for future wave-like promotion: per-environment rollout as the foundation, wave-like progression as a later capability.
...
- [x] 3.1 Define the conceptual source-structure roles building on the bootstrap installation contract: GitOps entrypoint, platform services (one management unit each), application services, environment binding, ordering and ownership boundaries.
- [x] 3.2 Define that environment binding must be separable per management unit.
- [x] 3.3 Keep source-structure roles conceptual and avoid mandating final directory paths or file names.
- [x] 3.4 Document that the repository boundary question from backlog 0010 (one-stop-shop vs template/example repos) is deferred to the first installable slice or source-layout implementation, with a clear forcing function.
...
- [x] 4.1 Document the contract-first packaging posture: concrete packaging can be chosen later provided it satisfies the management-unit contract.
- [x] 4.2 Identify Helm, Kustomize, and controller wrappers (Flux/Argo) as candidates without selecting one as final.
- [x] 4.3 State that the first management-unit implementation change is the forcing function for packaging selection.
...
- [x] 5.1 Explicitly defer the GitOps controller choice with rationale.
- [x] 5.2 Explicitly defer final packaging format choice with rationale.
- [x] 5.3 Explicitly defer additional platform services with rationale.
- [x] 5.4 Explicitly defer environment-specific directory structure with rationale.
...
- [x] 6.3 Link `docs/gitops-component-management.md` from `docs/README.md`.
...
- [x] 7.1 Run `openspec validate define-gitops-component-management --type change --strict --json --no-interactive` and resolve any errors.
- [x] 7.2 Run `openspec status --change define-gitops-component-management --json` and confirm all required artifacts are present.
- [x] 7.3 Verify no Kubernetes manifests, installation scripts, or technical skeleton directories were added.
- [x] 7.4 Verify required project vocabulary and two-axis model are preserved across all change artifacts.
