...
- **AND** the existing cluster entrypoint includes Flux `Kustomization` resources that reconcile the Kyverno controller before the smoke policy resources
...
Kubecrate SHALL provide a minimal Kyverno smoke policy that proves admission control enforcement. The proof SHALL include a `require-ns-label` ClusterPolicy in Enforce mode and an allowed fixture namespace that satisfies the policy.
...
- **THEN** CrateCheck reports the affected Kyverno checks as non-green
