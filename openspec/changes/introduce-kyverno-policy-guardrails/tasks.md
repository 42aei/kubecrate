...
- [x] 3.1 Define a `require-ns-label` ClusterPolicy that requires `kyverno-smoke-*` namespaces to carry `kubecrate.io/validated: "true"` in Enforce mode.
...
- A scoped namespace without the label is denied on creation with the exact policy reason by the disposable runner.
...
- [x] 4.3 Extend focused semantic tests with Kyverno check IDs, CEL behavior, exact RBAC, policy scope, entrypoint ordering, and authoritative render roots.
...
- [x] 5.2 Run `openspec validate introduce-kyverno-policy-guardrails --type change --strict --json --no-interactive` and resolve any errors.
- [x] 5.3 Run static rendering for all new Kyverno roots and the current kind-first local path entrypoint.
- [x] 5.4 Run repository manifest validation and kubeconform.
- [x] 5.5 Run focused and full relevant unit tests, including the exact runner behavioral suite.
...
- [ ] 5.8 Run the exact-candidate disposable JSON-only controlled red test; verify only the expected Kyverno check is red and unrelated checks remain green.
