...
- Add a narrowly scoped smoke policy: a `require-ns-label` ClusterPolicy that requires `kyverno-smoke-*` namespaces to carry the label `kubecrate.io/validated: "true"`, plus an allowed fixture namespace that satisfies the policy.
...
- Extend the exact-candidate JSON-only disposable runner to prove installation, controller health, policy readiness, an allowed admission, an unlabeled admission denied for the exact policy reason, and green → Kyverno-only controlled red → restored green CrateCheck status.
...
- Does not make Kyverno part of bootstrap installation acceptance or change the durable Flux `main` default and exact-candidate runner identity contract.
