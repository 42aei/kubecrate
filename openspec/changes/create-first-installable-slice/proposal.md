...
- **Resolve runtime layout**: Prior changes left source-structure roles conceptual. 0008 adopts `clusters/<cluster>/entrypoint` as the first GitOps reconciliation root, requires real platform services to use `platform-services/<service>/base/` plus `clusters/<cluster>/platform-services/<service>/` immediately, and allows `kubecrate-reconciliation-marker` to live directly in the concrete cluster path as validation material because it is not a platform service or application service. The first concrete cluster is `kind-dev-misc-local`, and temporary cluster-local platform service implementations are forbidden unless a later approved change includes a removal plan.
...
- Adds the first concrete runtime files to this repository: bootstrap installation manifests, Flux desired-state path, the official External-Secrets Operator chart rendered through Kustomize from `platform-services/external-secrets-operator/base/` plus `clusters/kind-dev-misc-local/platform-services/external-secrets-operator/`, ESO Kubernetes provider configuration, Seed Secrets bootstrap materialization from `.env`, the `kubecrate-reconciliation-marker` proof under the concrete cluster path for `kind-dev-misc-local`, and kind validation plumbing (kind config, Make targets or equivalents, evidence commands).
...
- Resolves the deferred runtime layout: `clusters/<cluster>/entrypoint` is required now, and real platform services use `platform-services/<service>/base/` plus `clusters/<cluster>/platform-services/<service>/` immediately.
