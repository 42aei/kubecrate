...
- Prove local TLS certificate issuance from a self-signed CA through a ClusterIssuer chain and consume the issued certificate on the existing Envoy Gateway HTTPS path.
...
- Redesigning the existing Envoy Gateway path beyond adding the minimum HTTPS listener, cross-namespace Secret grant, and route binding needed to consume the issued certificate.
...
### Envoy Gateway consumes the issued certificate

The existing smoke Gateway adds an HTTPS listener for `cratecheck.local`. Its certificate reference targets `Secret/cratecheck-tls` in the `cratecheck` namespace and is authorized by a narrow `ReferenceGrant`. The existing HTTPRoute attaches to both the HTTP and HTTPS listeners. The direct disposable kind runner validates HTTPS with the issued CA and hostname; insecure TLS is not accepted as proof.
