...
### Requirement: Dedicated QA source for disposable Flux clusters
Kubecrate SHALL let the direct QA runner reconcile a verified dedicated branch in a disposable cluster without changing the committed canonical source reference.
...
- **THEN** the direct runner renders the verified QA branch before its initial entrypoint apply
- **AND** verifies the reconciled Flux artifact commit
