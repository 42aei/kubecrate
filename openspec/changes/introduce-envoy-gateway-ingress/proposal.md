...
- **Add minimal CrateCheck RBAC for Envoy Gateway resources**: reuse existing HelmRelease access and add read access to `gatewayclasses`, `gateways`, and `httproutes`.
- **Extend direct disposable-cluster QA**: render the dedicated QA branch into the in-memory Flux source and prove ingress plus an Envoy-only red/restore cycle without changing the committed `main` source reference.
