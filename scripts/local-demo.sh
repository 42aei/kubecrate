#!/usr/bin/env bash
# Retained, anonymous kind-first Kubecrate demo lifecycle.
set -euo pipefail
...
normalize_url(){ local raw="$1" path; case "$raw" in git@github.com:*) path="${raw#git@github.com:}";; ssh://git@github.com/*) path="${raw#ssh://git@github.com/}";; https://github.com/*) path="${raw#https://github.com/}";; http://github.com/*) path="${raw#http://github.com/}";; *) fail "cannot derive supported public GitHub URL; set KUBECRATE_LOCAL_SOURCE_URL";; esac; path="${path%.git}"; [[ "$path" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]] || fail "ambiguous public repository path"; printf 'https://github.com/%s.git\n' "$path"; }
GIT_USERNAME=""; GIT_PASSWORD=""
capture_basic_auth(){
  GIT_USERNAME="${KUBECRATE_LOCAL_GIT_USERNAME:-}"; GIT_PASSWORD="${KUBECRATE_LOCAL_GIT_PASSWORD:-}"
  if test -z "$GIT_USERNAME" || test -z "$GIT_PASSWORD"; then
    local host filled; host="$(printf '%s\n' "$SOURCE_URL" | sed -E 's#^https?://([^/]+)/.*$#\1#')"
    filled="$(printf 'protocol=https\nhost=%s\n\n' "$host" | bounded_probe git credential fill 2>/dev/null || true)"
    test -n "$GIT_USERNAME" || GIT_USERNAME="$(printf '%s\n' "$filled" | sed -n 's/^username=//p')"
    test -n "$GIT_PASSWORD" || GIT_PASSWORD="$(printf '%s\n' "$filled" | sed -n 's/^password=//p')"
  fi
  test -n "$GIT_USERNAME" && test -n "$GIT_PASSWORD" || fail "private source requires basic-auth credentials; set KUBECRATE_LOCAL_GIT_USERNAME and KUBECRATE_LOCAL_GIT_PASSWORD or store them via git credential"
}
derive_source(){
  ...
  local advertised rc count isolated_home; isolated_home="$(mktemp -d)"; set +e
  if test -n "${KUBECRATE_LOCAL_GIT_BASIC_AUTH:-}"; then
    capture_basic_auth
    advertised="$(env -i PATH="$PATH" HOME="$isolated_home" CALL_LOG="${CALL_LOG:-}" FAKE_SECRET_SENTINEL="${FAKE_SECRET_SENTINEL:-}" GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_TERMINAL_PROMPT=0 KUBECRATE_LOCAL_GIT_USERNAME="$GIT_USERNAME" KUBECRATE_LOCAL_GIT_PASSWORD="$GIT_PASSWORD" timeout --signal=TERM --kill-after=2s "$PROBE_TIMEOUT" git -c credential.helper='!f() { echo "username=$KUBECRATE_LOCAL_GIT_USERNAME"; echo "password=$KUBECRATE_LOCAL_GIT_PASSWORD"; }; f' ls-remote --heads "$SOURCE_URL" "refs/heads/$SOURCE_REF" 2>&1)"; rc=$?; rm -rf "$isolated_home"; set -e
    test $rc -eq 0 || { printf '%s\n' "$advertised" >&2; FAIL_RC=$rc fail "selected source is not accessible with the supplied basic-auth credentials"; }
  else
...
validate_source_render(){ PHASE=source-render; local mode=(); test -n "${KUBECRATE_LOCAL_GIT_BASIC_AUTH:-}" || mode=(--anonymous); bounded_probe kustomize build "$ENTRYPOINT_ROOT" | bounded_probe python3 scripts/render-direct-flux-source.py --https-url "$SOURCE_URL" --branch "$SOURCE_REF" "${mode[@]}" >/dev/null || fail "exact Flux source rendering is impossible"; }
...
bootstrap(){ PHASE=flux-bootstrap; assert_context; bounded_operation "$WAIT_LONG" helm upgrade --install flux-system "$FLUX_CHART" --kube-context "$CONTEXT" --version "$FLUX_CHART_VERSION" --namespace "$FLUX_NAMESPACE" --create-namespace -f "$FLUX_HELM_VALUES" --wait --timeout "$WAIT_LONG"; for d in source-controller kustomize-controller helm-controller; do bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" wait --for=condition=Available "deployment/$d" -n "$FLUX_NAMESPACE" --timeout="$WAIT_SHORT"; done; assert_context; local mode=(); test -n "${KUBECRATE_LOCAL_GIT_BASIC_AUTH:-}" || mode=(--anonymous); if test -n "${KUBECRATE_LOCAL_GIT_BASIC_AUTH:-}"; then bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" -n "$FLUX_NAMESPACE" create secret generic "$SYNC_NAME" --from-literal=username="$GIT_USERNAME" --from-literal=password="$GIT_PASSWORD" --dry-run=client -o yaml | bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" apply -f -; fi; bounded_probe kustomize build "$ENTRYPOINT_ROOT" | bounded_probe python3 scripts/render-direct-flux-source.py --https-url "$SOURCE_URL" --branch "$SOURCE_REF" "${mode[@]}" | bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" apply -f -; bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" wait --for=condition=Ready "helmrelease/$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_SHORT"; bounded_operation "$WAIT_LONG" flux --context "$CONTEXT" reconcile source git "$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_LONG"; bounded_operation "$WAIT_LONG" flux --context "$CONTEXT" reconcile kustomization "$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_LONG"; }
status_checks(){
