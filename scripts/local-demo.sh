...
validate_source_render(){ PHASE=source-render; bounded_probe kustomize build "$ENTRYPOINT_ROOT" | bounded_probe python3 scripts/render-direct-flux-source.py --https-url "$SOURCE_URL" --branch "$SOURCE_REF" --anonymous >/dev/null || fail "exact anonymous Flux source rendering is impossible"; }
...
bootstrap(){ PHASE=flux-bootstrap; assert_context; bounded_operation "$WAIT_LONG" helm upgrade --install flux-system "$FLUX_CHART" --kube-context "$CONTEXT" --version "$FLUX_CHART_VERSION" --namespace "$FLUX_NAMESPACE" --create-namespace -f "$FLUX_HELM_VALUES" --wait --timeout "$WAIT_LONG"; for d in source-controller kustomize-controller helm-controller; do bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" wait --for=condition=Available "deployment/$d" -n "$FLUX_NAMESPACE" --timeout="$WAIT_SHORT"; done; assert_context; bounded_probe kustomize build "$ENTRYPOINT_ROOT" | bounded_probe python3 scripts/render-direct-flux-source.py --https-url "$SOURCE_URL" --branch "$SOURCE_REF" --anonymous | bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" apply -f -; bounded_operation "$WAIT_SHORT" kubectl --context "$CONTEXT" wait --for=condition=Ready "helmrelease/$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_SHORT"; bounded_operation "$WAIT_LONG" flux --context "$CONTEXT" reconcile source git "$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_LONG"; bounded_operation "$WAIT_LONG" flux --context "$CONTEXT" reconcile kustomization "$SYNC_NAME" -n "$FLUX_NAMESPACE" --timeout="$WAIT_LONG"; }
status_checks(){
